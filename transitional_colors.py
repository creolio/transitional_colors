...
def adjust_color_to_luminance(color: List[int], lower_limit: float = 0, upper_limit: float = 1) -> Tuple[int, int, int]:
...
def generate_intermediary_colors(
    ...
    for i in range(1, number_of_colors_between + 1):
        ...
        color = adjust_color_to_luminance(color, lower_limit, upper_limit)
        colors.append(tuple(color))
    return colors


def format_color_code(
    color: Tuple[int, int, int], lower_limit: float = 0, upper_limit: float = 1
) -> Tuple[int, int, int]:
    return adjust_color_to_luminance(color, lower_limit, upper_limit)
...
def generate_transitional_colors_list(
    ...
    colors_formatted = [format_color_code(color, lower_limit, upper_limit) for color in colors]
    color_range = []
    for i in range(len(colors_formatted) - 1):
        color_range.extend([colors_formatted[i]])
        color_range.extend(
            generate_intermediary_colors(
                colors_formatted[i], colors_formatted[i + 1], number_of_colors_between, lower_limit, upper_limit
            )
        )
...
def generate_transitional_colors_dictionary(
    ...
    colors_list = generate_transitional_colors_list(colors, number_of_colors_between, lower_limit, upper_limit)
